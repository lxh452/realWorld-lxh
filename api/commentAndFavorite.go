...
// 获取文章评论
func GetCommentFromArticle(c *gin.Context) {
	//获取文章摘要
	slug := c.Param("slug")
	//绑定结构体
...
// 在文章添加评论
func AddcommentToArticle(c *gin.Context) {
...
func DeleteCommentFromArticle(c *gin.Context) {
	slug := c.Param("slug")
	...
	comment := service.ArticleServiceApp
	comment.DeleteCommentFromArticle(slug, uint(commentId), claims.Id)
}

// 添加文章到喜欢中
func AddArticleIntoFavorite(c *gin.Context) {
	slug := c.Param("slug")
	claims, err := utils.GetClaims(c)
	if err != nil {
		resp.FailWithMessage(err.Error(), c)
...
// 删除文章到喜欢中
func DeleteArticleFromFavorite(c *gin.Context) {
	slug := c.Param("slug")
	claims, err := utils.GetClaims(c)
	if err != nil {
		resp.FailWithMessage(err.Error(), c)
